#'@title:  "Community analysis:: altitude"
#'@author: "R Alcala"
#'@date:   "16/03/2021"
#'@output: "Comm Index"
#'
...
# Virome 2:: long table with taxomic data by virome species cluster 
#            & relative sequence abundance :: RPKM normalized
virome=virome2
   
as_tibble(virome)
virome1 <- ddply(virome, .(Acronym, altzones), summarise, Coverage=mean(RPKM_mean))
virome.m <- tidyr::spread(virome1, altzones, Coverage,  drop=TRUE , fill = 0)
...
# a network
# vg <-  graph.incidence(virome, weighted=T)
# v2 <- t(virome) %*% virome
# plot(vg, layout_nicely(vg))

#---- Beggings:
#---- communty analysis: CommEcol
