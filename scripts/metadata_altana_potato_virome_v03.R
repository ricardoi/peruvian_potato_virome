...
peruvian_potato_virome <- read.csv("peruvian_potato_virome_vsc-rpkmx_ViNAtq_ed_Mar14.csv")[-1]
...
as_tibble(dat)
datall <- dat %>%
         select(IDs, Family, Genus, pSpecies, Acronym, Length_mean, Bases_mean,
         Coverage_mean, Reads_mean, RPKM_mean, RPKMlog2,  Frags_mean, FPKM_mean)
  datball <- ddply(datall, .(IDs, Acronym), summarise, cov=mean(RPKM_mean))
